...
import Spinner from '../components/Spinner'; // Assuming this is correctly imported
import { Link } from 'react-router-dom';
import { AiOutlineEdit } from 'react-icons/ai';
import { BsInfoCircle } from 'react-icons/bs';
import { MdAddBox, MdOutlineDelete } from 'react-icons/md';  // Corrected icon name
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setBooks(response.data.data); // Correctly set books from API response
        setLoading(false);
      })
      .catch((error) => {
    ...
    <div className='p-4'>
      <div className='flex justify-between items-center'>
        ...
        <Link to='/books/create'>
          <MdAddBox className='text-sky-800 text-4xl' />  {/* Updated icon */}
        </Link>
      </div>
      {loading ? (
        <Spinner />
